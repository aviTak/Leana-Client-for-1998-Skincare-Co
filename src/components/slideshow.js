        ...
        if(mat.length !== 0){
            ...
            let a = mat.map(asset => (
                ...
                <tr key = {asset.id}>
                    ...
                    <td className = 'hover one'><Link style={{textDecoration: 'none'}} to = {'/slideshow/title/' + asset.id}><div id={asset.id} style = {{width: '100%'}}>{asset.heading}</div></Link></td>
                        ...
                        <div className = 'search__box'>
                            <form id = "search" onSubmit = { this.submitForm } autoComplete="off">
                                ...
                                <span className = "search__field">
                                    <input value={this.state.search} inputMode = "search" required type = "text" placeholder = "Search..." onChange = { (e) => this.setState({search: e.target.value}) } />
                                </span>
