...
class Subscriber extends React.Component {
    ...
    componentDidMount(){
        try{
            document.getElementById('subscribers').classList.add('selected');
        } catch(e){}
    ...
    componentWillUnmount(){
        try{
            document.getElementById('subscribers').classList.remove('selected');
            clearTimeout(time);
        } catch(e){}
    }
        ...
        if(mat.length !== 0){
            ...
            let a = mat.map(asset => (
                ...
                <tr key = {asset.id}>
                    ...
                    <td className = 'hover qot one'><div id={asset.id} style = {{width: '100%'}}>{asset.email}</div></td>
